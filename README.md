...
这是一场运气游戏的实现，参与者将轮流对自己开枪，直到剩下最后一名幸存者。

#### 可用指令

| 指令格式 | 功能描述 |
| :--- | :--- |
| **`#俄罗斯转盘[数量]`** | 在群里发起一场俄罗斯转盘游戏。<br>数量为可选参数（1-5），代表左轮手枪中的子弹数量。<br>若不指定，则默认为 **1** 颗子弹。 |
| **`#加入转盘`**<br>**`#加入轮盘`** | 加入当前群聊中正在等待开始的游戏。 |
| **`#退出转盘`** | 在游戏**开始前**退出赌局。房主退出将直接解散游戏。 |
| **`#开始转盘`** | 由房主手动提前开始游戏，无需等待30秒倒计时。 |
